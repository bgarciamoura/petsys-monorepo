...
const getAllPackageJsons = async (workspaces) => {
    ...
    return Promise.all(
        ...
        .then((allPackages) => allPackages.reduce((flattendArray, files) => [...flattendArray, ...files], []))
        .then((allPackages) =>
            allPackages.filter(
                (packageName) =>
...
const getReferencesFromDependencies = (configName, { packageDir }, packageName, packagesMap, verbose) => {
    const packageJsonFilePath = path.join(packageDir, PACKAGE_JSON);

    const { dependencies = {}, peerDependencies = {}, devDependencies = {} } = require(packageJsonFilePath);
            ...
            if (packagesMap.has(dependency)) {
                ...
                if (detectedConfig !== null) {
                    return [
                        ...
                        {
                            path:
                                detectedConfig !== TSCONFIG_JSON
                                    ? path.join(relativePath, detectedConfig)
                                    : relativePath,
                        },
                    ];
                }
...
const execute = async ({ cwd, verbose, discardComments, check, configName }) => {
    let changesCount = 0;
    ...
    if (!workspaces && fs.existsSync(path.join(cwd, 'pnpm-workspace.yaml'))) {
        const pnpmConfig = yaml.load(fs.readFileSync(path.join(cwd, 'pnpm-workspace.yaml')));
        workspaces = pnpmConfig.packages;
    }

    if (!workspaces) {
        throw new Error('could not detect yarn workspaces or lerna in this repository');
    }
    ...
    packagesMap.forEach((packageEntry, packageName) => {
        ...
        if (detectedConfig) {
            rootReferences.push({
                path: path.join(
                    path.relative(process.cwd(), packageEntry.packageDir),
                    detectedConfig !== TSCONFIG_JSON ? detectedConfig : ''
                ),
            });
            ...
            changesCount += updateTsConfig(detectedConfig, references, discardComments, check, packageEntry);
        } else {
            // eslint-disable-next-line no-console
            console.log(
                `NO ${
                    configName === TSCONFIG_JSON ? configName : `${configName} nor ${TSCONFIG_JSON}`
                } for ${packageName}`
            );
        }
    });
    ...
    changesCount += updateTsConfig(configName, rootReferences, discardComments, check);
...
const run = async () => {
    try {
        ...
        if (defaultOptions.check && changesCount > 0) {
